...
## `version.json` is not in this repo

The update manifest the Android direct-channel app reads
(`https://gogba.xyz/version.json`) is **generated by the app repo** at release
time, by `scripts/build_android_direct.sh`, and deployed straight from there.

It used to live here and was edited across repos by that script. It was moved
because a full-site deploy from this repo would overwrite the freshly released
manifest with a stale copy. Do not re-add it here.
